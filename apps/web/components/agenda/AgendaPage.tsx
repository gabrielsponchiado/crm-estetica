...
export function AgendaPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-112px)] gap-4">
      ...
      <div className="bg-white p-2.5 px-4 rounded-2xl border border-slate-200 shadow-xs grid grid-cols-1 md:grid-cols-3 items-center gap-3 shrink-0">
        <div className="flex items-center gap-3 justify-start">
          <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200/60 shrink-0">
            <button
              onClick={handlePrevious}
              title="Anterior"
              className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white hover:shadow-xs text-slate-600 transition font-bold text-xs cursor-pointer"
            >
              ◀
            </button>
            <button
              onClick={handleToday}
              className="px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-white hover:shadow-xs rounded-lg transition cursor-pointer"
            >
              Hoje
            </button>
            <button
          ...
          <span className="text-xs sm:text-sm font-bold text-slate-800 capitalize truncate">
            {getHeaderTitle()}
          </span>
        </div>

        <div className="flex justify-center">
          <div className="bg-slate-100 p-1 rounded-xl flex gap-0.5 text-xs font-medium text-slate-600 border border-slate-200/60 shrink-0">
            <button
        ...
        <div className="hidden lg:flex items-center justify-end gap-3 text-xs font-medium text-slate-600">
      ...
      <div className="flex-1 min-h-0 bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden flex flex-col p-3">
        {viewMode === "day" && (
          <div className="flex-1 grid grid-rows-9 divide-y divide-slate-100 h-full overflow-y-auto">
            {timeSlots.map((time) => {
              const appointment = appointments.find(
                (app) =>
                  app.date === formattedSelectedDate && app.startTime === time,
              );
              ...
              return (
                <div
                  key={time}
                  className="flex items-center gap-3 px-2 group hover:bg-slate-50/60 transition min-h-[52px]"
                >
                      ...
                      >
                        ...
                        >
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="px-2.5 py-1 rounded-lg bg-white/90 hover:bg-white text-emerald-700 border border-emerald-200 text-xs font-semibold transition"
                          >
                            💬 Whats
                          </a>
                        </div>
